document.querySelectorAll(".projectinfo").forEach(function (projectinfo) {
  projectinfo.addEventListener("mouseenter", function (event) {
    ...
    for (let i = 1; i < 13; i++) {
      document
        .querySelector(`.project-img${i}`)
        .classList.remove(`project-img${i}-transition`);
    }
    document
      .querySelector(`.project-img${projectnumber}`)
      .classList.add(`project-img${projectnumber}-transition`);
  });
